...
The idea of including infinitary information in strategies is not new, but this particular presentation, though closely related to that of \cite{LevyGsInfinite}, but this is the first time it has been used to establish a full abstraction result for may and must testing.
...
We define a Felleisen-style \emph{evaluation context} $E$ inductively as follows.
\begin{IEEEeqnarray*}{RCL}
...
For any game that is the denotation of an Idealized Algol type, this enumeration will be the natural one as described above.
...
Given a strategy $\sigma$ for a game $A$, we say that $s\in\sigma$ is \emph{unreliable} if player $P$ has a strategy, starting from $s$ and staying at all times within $T_\sigma$, that ensures that we eventually reach some play in $D_\sigma$.  
...
For example, our category $\G_{ND}$ is a sensible game semantics of PCF with countable nondeterminism, as is the Tsukada-Ong category $\G_{TO}$.  

Using this condition and Lemma \ref{lem:pcf-soundness-lemma}, it is easy to prove the following version of adequacy, which is sufficient for the purpose of proving full abstraction.

\begin{theorem}[Adequacy for nondeterministic PCF]
  Let $\C$ be a sensible model of nondeterministic PCF, and define $\deno -$ to be given by the functor $J$ and the morphism $\top_\bN$ in the obvious way.
...
Tsukada and Ong prove compact definability from first principles, rather than using a factorization result, and it is not immediate how one defines a recursive element of their model.  
Nevertheless, we can have some confidence that such a result is possible, which will allow our results to be carried over to stateless languages.

It would be interesting to see whether our adequacy proof can be applied to effects other than nondeterminism; although most effects do not engender a failure of continuity in the same way, a similar approach could help us use an adequacy result from one language to prove adequacy for another language, rather than proving it from first principles each time.

Lastly, our proof has thrown more light on the important relation between nondeterminism and state.  
As we have discussed, stateful languages (and visible strategies) pose fewer problems for modelling nondeterminism than do stateless languages (and innocent strategies), but the fact that we needed to use the stateful structure of the language in our proof of Lemma \ref{lem:soundness-lemma} suggests that there is a deeper relationship between the two, which we should explore in future work.
...
\begin{acks}                            %% acks environment is optional
  ...
  No.~\grantnum{GS501100000690}{EP/K018868/1}.   I am grateful to Martin Hyland for an interesting conversation that helped me develop some of this material.
\end{acks}
