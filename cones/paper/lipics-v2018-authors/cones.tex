...
\begin{abstract}
  ...
  We discuss how our techniques may also be applied to yield a proof of computational adequacy for a model of PCF with unbounded nondeterminism, based on the construction by Hyland and Ong.
\end{abstract}
...
We shall assume that our arenas are \emph{enumerated}; that is, that the set $M_A$ comes equipped with a partial surjection $\bN\to M_A$. 
We assume that this enumeration is defined on the flat arenas and extended to the connectives in a natural way.
...
We say that such a string is \emph{recursive} if it corresponds, via the enumeration on $M_A$ to a recursive function $\bN\to\bN$.  
...
If $\sigma$ is a complete strategy for a game $A$, we say that $\sigma$ is \emph{recursive} if $T_\sigma\cap P_A$ and $D_\sigma\cap P_A$ are recursively enumerable subsets of $\omega^\omega$ (under the enumeration of $M_A$).  
Here, we have thrown away the infinite plays in $T_\sigma$ and $D_\sigma$, but we do not lose any information because $\sigma$ is complete.

If $\sigma$ is locally complete, we say that $\sigma$ is \emph{recursive} if $\sigma$ is the union of complete strategies $\sigma_1,\sigma_2,\cdots$, where the $\sigma_i$ are recursive and the map $i\mapsto \sigma_i$ is a recursive function $\bN \to (\bN \to \bN) \to 2$.
...
Note that these last two lemmas may be cast entirely in the model of Idealized Algol given in \cite{SamsonGuyIAPassive}, since they only refer to the denotations of deterministic terms.  
...
This definition is intimately connected to Lemmas \ref{lem:soundness-lemma} and \ref{lem:soundness-converse} in the following sense: if $\sigma_M\from\bN\to\bC$ is a strategy, then there exists some $\sigma_M$ such that $D_{\sigma;\sigma_M}\ne\emptyset$ if and only if $\sigma$ is not winning.
...
The principal difficulties in modelling nondeterministic stateless languages were made by Tsukada and Ong in \cite{TsukadaSheaves}, where they discovered how to define an innocent nondeterministic strategy by retaining `branching time information' in strategies.  
...
However, since there is no such model in the literature at the moment, we shall have to be a bit more general in examining how our techniques can be used to model nondeterministic PCF.  
Our main new contribution is the proof of adequacy, and our methods can be applied to PCF as well.  
...
Corollary \ref{cor:adequacy} reduces the proof of adequacy to a combinatorial check on morphisms from $\bN\to\bC$ on strategies in the well-known category $\G_{vis}^\oc$.
