...
I (Gowers) am grateful to Martin Hyland for our conversation that helped me to develop some of this material.
We are also grateful for the comments made by the anonymous referees for helping to clarify certain points, elaborate on technical difficulties and point to similarities with other parts of the literature.\vspace{24pt}}%optional
...
\newcommand{\oes}{\sim}
\newcommand{\orr}{\textsf{ or }}
\newcommand{\lunit}{\textsf{lunit}}
...
This is strictly more powerful than finitary choice nondeterminism\footnote{Using recursion, we can build a program out of finite nondeterminism that can produce arbitrarily large natural numbers; however, this program also admits the possibility of divergence, unless we are able to insist on fairness.}.
...
This is an important observation to make about concurrent programming, because the task of modelling unbounded nondeterminism is difficult -- indeed, considerably more so than that of modelling bounded nondeterminism.  
...
Park \cite{park1980semantics} shows that these problems can be surmounted if we use a weaker version of continuity (e.g., $\omega_1$- rather than $\omega$-continuity), but the failure of composition to be continuous is a problem in itself for semanticists, for whom continuity is often a key ingredient in proofs of computational adequacy and full abstraction.
...
Then we write $M\lmam N$ if for all contexts $C[-]$ of ground type with a hole of type $T$, we have
\begin{gather*}
...
\begin{itemize}
  ...
  \item $\ts_A$ is an \emph{enabling relation} between $M_A+\{*\}$ and $M_A$ satisfying the following rules:
    \begin{itemize}
      \item If $a\ts_A b$, then $\lambda_A^{OP}(a)\neq\lambda_A^{OP}(b)$.  
...
What this illustrates is the point made by Park in \cite{park1980semantics} and \cite{park1981concurrency}: namely, that we can no longer deduce the infinitary behaviour of a strategy by looking at the limits of its finite plays; instead, we need to keep track of infinite sequences of moves explicitly, in the style of \cite{RoscoeCspInfinite} and \cite{LevyGsInfinite}.
...
We say such a string is \emph{recursive} if it corresponds, via the enumeration on $M_A$, to a pair of recursive functions $\bN\to\bN$ -- one giving the sequence of moves and the other giving the justification relation.
...
We shall represent a strategy using two sets: a set $T_\sigma$ of \emph{traces}, which takes the role of the plays that may occur in the strategy (as in the usual definition of a deterministic strategy), and a second set $D_\sigma$ of \emph{divergences}; i.e., $O$-positions at which the strategy may elect to diverge.  
In order to model observational equivalence more closely, we shall require $D_\sigma$ to be postfix-closed, since observable contexts cannot detect divergences that occur after the program might already have diverged: consider, for example, the terms $\Omega_{\nat\to\nat}\orr(\lambda n.n)$ and $\Omega_{\nat\to\nat}\orr(\lambda n.(n\orr\Omega_\nat))$ (where we have defined $M\orr N$ to be $\IfO\;\wn\;M\;N$).

For technical reasons we keep track of infinite plays in both $T_\sigma$ and $D_\sigma$, with the rule that any infinite play in $T_\sigma$ must be contained in $D_\sigma$ (since it clearly corresponds to a divergent evaluation).  
We will require that every divergence arise from a trace; i.e., if every play in $D_\sigma$ must have some prefix that is contained in both $T_\sigma$ and $D_\sigma$.  
A consequence of this is that if $d\in D_\sigma$ is infinite and has no finite prefixes in $D_\sigma$, then it must also be contained in $T_\sigma$.  
Not too much importance should be given, however, to the presence or absence of infinite plays in $T_\sigma$: it is quick to show that once we pass to the intrinsic quotient, any such distinction vanishes.
...
\begin{itemize}
  ...
  \item $D_\sigma\subset \overline{P_A}$ is a postfix-closed set of plays in $\overline{P_A}$ that either end with an $O$-move or are infinite.  
    ...
    \begin{description}
      \item[Divergences come from traces] If $d\in D_\sigma$ then there exists $s\prefix d$ such that $s\in T_\sigma\cap D_\sigma$.
...
Note that there is no need to consider separately, as Harmer and McCusker do, divergences that arise through `infinite chattering': in our model, we will see that a case of infinite chattering between strategies $\sigma$ and $\tau$ is itself a legal interaction between the two strategies, which is necessarily divergent (because it is infinite) and therefore gives rise to some divergence in $\sigma;\tau$.
...
\begin{definition}
  Let $\sigma$ be a strategy for a game $A$.  
  We say that $\sigma$ is \emph{locally complete} if it may be written as the union of complete strategies; i.e., there exist $\sigma_i$ such that $T_\sigma=\bigcup T_{\sigma_i}$ and $D_\sigma=\bigcup D_{\sigma_i}$.  
  Note that since $T_\sigma$ and $D_\sigma$ are countable sets (because there are countably many recursive plays), this union may be taken to be countable.
\end{definition}

It will be slightly more convenient to use an equivalent definition, based on unions of \emph{deterministic} strategies, which are a special case of complete strategies.

\begin{definition}
  We say that a strategy $\sigma$ for a game $A$ is \emph{deterministic} if
  \begin{itemize}
    \item it is complete;
    \item if $sa,sb$ are $P$-plays in $T_\sigma$ then $a=b$ and the justifier of $a$ is the justifier of $b$;
    \item if $s\in D_\sigma$ then either $s$ is infinite or there is no $a$ such that $sa\in T_\sigma$.
  \end{itemize}
\end{definition}

We say that a strategy $\sigma$ is \emph{lively} or \emph{locally deterministic} if there exists a collection of deterministic strategies $\sigma_i$ such that $T_\sigma=\bigcup T_{\sigma_i}$ and $D_\sigma=\bigcup D_{\sigma_i}$.  
It is clear that a strategy is lively if and only if it is locally complete, and that the collection of $\sigma_i$ may again be taken to be countable.

From now on, we will use \emph{strategy} to mean \emph{lively} (or \emph{locally complete}) \emph{strategy}. This means that we will need to show that the composition of lively strategies is again lively.  
...
After passing to the intrinsic quotient, the resulting category $\G^\oc_{D,vis}$ of games and deterministic visible strategies is a fully abstract model of Idealized Algol \cite{SamsonGuyIAPassive}.
...
If $\sigma$ is lively, we say that $\sigma$ is \emph{recursive}, and if $\sigma$ is the union of complete recursive strategies $\sigma_1,\sigma_2,\cdots$, where the map $i\mapsto \sigma_i$ is a recursive function $\bN \to (\bN \to \bN) \to 2$.

Note that there are plenty of strategies that we want that are not the union of a recursive sequence of \emph{deterministic} strategies -- for example, the strategy on $(\bN\to\bC)\to\bC$ that calls its natural-number argument infinitely many times is complete and has no $O$-branching, but its infinite traces include every recursive sequence of natural numbers.

Using these definitions, it seems to be hard to prove that the composition of recursive strategies is itself a recursive strategy: the tricky point is to show that the decomposition into complete strategies may still be taken to be given by a recursive strategy.  
The example in the previous paragraph shows that we cannot use the same proof as we did in the non-recursive case, which used deterministic strategies.  
Fortunately, we do not need to be able to show that the composition of recursive strategies is recursive in order to prove our full abstraction result, so we leave this problem for future work.
...
\begin{proof}
  ...
  For each $O$-position $s\in T_\sigma$, we have some set $B$ of possible replies to $s$, which we order as $b_1,b_2,\cdots$, where $\code_A(b_1)<\code_A(b_2)<\cdots$.  
  ...
  \begin{itemize}
    ...
    \item If $j = 0$ and $s\in D_\sigma$, then play nothing, and put the resulting play inside $D_{\Det(\sigma)}$.  
      Otherwise, play $b_1$.
  \end{itemize}
...
We model the language compositionally, using denotations as in \cite{mcCHFiniteND} for the nondeterministic constants and modelling $\wn$ using the strategy $\top_{\bN}\from \bN$.  
...
\begin{lemma}
  The term $C[\wn]$ has the same denotation as the term $(\lambda n.C[n])\wn$.
\end{lemma}

\begin{proof}
  This is a straightforward argument by structural induction on $C$, and the constant $\top_\bN$ does not really play a role.  
  We prove inductively on $T$ that if $\Gamma\ts C[\wn]\from T$ is a term-in-context, then its denotation may be given by the following composite.
  \[
    \deno{\Gamma}
    \xrightarrow{\lunit;(\top_{\bN}\times\id)}
    \bN\times\deno{\Gamma}
    \xrightarrow{\deno{\Gamma,n\from\nat\ts C[n]\from T}}
    \deno{T}
    \]
\end{proof}
...
Corollary \ref{cor:adequacy} is very general, and is intended to be applied in multiple situations.  
In particular, it may be applied to a game semantics in which we define a `nondeterministic visible strategy' on a game $A$ to be a deterministic visible strategy for $\bN\to A$, up to a suitable equivalence relation.  
This model is an example of a much more general construction that is the subject of ongoing research by the authors.  
In this sense, our main model based on nondeterministic strategies is not necessary in order to obtain our full abstraction result.  
Nevertheless, we felt it important to give a model based on nondeterministic strategies, since these are the `natural' game semantic interpretation of nondeterminism.
...
For full abstraction, we need to take the intrinsic quotient in order to identify, for example, the terms $\lambda n.\Omega$ and $\lambda n.\IfO n\;\Omega\;\Omega \from \nat\to\nat$.  
These terms are observationally equivalent, but their denotations are not equal; for example, $q\in D_{\deno{\lambda n.\Omega}}$, but $q\not\in D_{\deno{\lambda n.\IfO n\;\Omega\;\Omega}}$.
...
We say that $s$ is \emph{unreliable} if player $P$ has a strategy for the game starting at $s$ that ensures that the (possibly infinite) limiting play is in $D_\sigma$.  
...
Tsukada and Ong's primary model was based on sheaves over a site of plays, but they also give a more direct way of characterizing nondeterministic innocence, based on ideas by Levy \cite{levy2014morphisms}.
