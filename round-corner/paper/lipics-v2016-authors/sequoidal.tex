...
Games models of pure functional programming languages such as PCF (\cite{ajmPcf} and \cite{hoPcf}, for example) tend to include the requirement that strategies be \emph{history-free}\footnote{In fact, the model in \cite{hoPcf} and other models following the `Hyland-Ong' tradition use \emph{innocent} strategies, rather than history free strategies to model statelessness.  We will focus on AJM-style Game Semantics in this paper, so we shall focus on history-freeness vs. history sensitivity rather than innocence.}: that is, the response to a particular position may depend only upon the last move played, and not upon what has gone before.  This makes sense for stateless languages, where the program should have no `memory' to recall what has gone before.  

If history-free strategies are the game-semantic interpretation of stateless programs, then history-sensitive strategies should be the game-semantic interpretation of stateful programs, and this is indeed how they are used: Abramsky and McCusker's model of Idealized Algol \cite{SamsonGuyIAPassive}, for example, makes use of history-sensitive strategies in order to model stateful programs.  However, this idea of state can be difficult to work with, because it is completely \emph{implicit} in the strategy.  Let us, for example, try to model a simple storage cell.  Let $X$ be a set of values and, as in \cite{SamsonGuyIAPassive}, let $\Var[X]$ be the game with maximal plays $rx$ and $w_x\checkmark$, where $x$ ranges over $X$.  $rx$ corresponds to a $\read$ method: player $O$ plays $r$ and expects to receive the value $x$ currently in the cell, while $w_x\checkmark$ corresponds to a write method: player $O$ writes $x$ into the cell and then player $P$ confirms that the write has taken place.

It is now clear that we have a strategy $\cell$ on $\oc \Var[X]$ corresponding to a storage cell: whenever player $O$ plays $w_x$, respond with $\checkmark$, and whenever player $O$ plays $r$, respond with the last $x$ such that $w_x$ was played (if the cell is uninitialized -- i.e., if $w_x$ has never been played for any $x$ -- then we return either nothing or some specified initial value, depending on what we are trying to model).  This strategy is clearly history sensitive and represents a stateful program; however, it is tricky to reason about the statefulness of $\cell$, because there is no object explicitly representing the state: the state in $\cell$ is \emph{implicit}, and it appears that we need to resort to combinatorial arguments in order to reason about it.

Contrast this with the handling of state in traditional domain-theoretic semantics, where we represent a stateful program as a function $W \times X \to W \times Y$, or a \emph{state transformer}, where $W$ is a built-in store of state that can be reasoned about directly.  In 2002, one of the authors introduced the game-semantic equivalent of a state-transformer by defining a new operation on games -- the \emph{sequoid} $\blank\sequoid\blank$\cite{laird02}.  The game $A\sequoid B$ is the same as the game $A\tensor B$, except that the first move must take place in $A$.  We now model a state transformer for a program as a morphism $S \to A \sequoid S$ -- i.e., a \emph{coalgebra} for the functor $A\sequoid\blank$ -- where $A$ is the program and $S$ is the changed state.  The justification for the order relation on the sequoid operator is that the state should only change in response to some action by the environment.  

For example, let $X$ be the game with maximal plays $rx$ for $x\in X$.  Then we may define a state transformer for $X \implies \Var[X]\sequoid X$ as follows: play must start in the game $\Var[X]$.  If player $O$ starts with the move $r$, then request a value of $x$ on the left and then copy it into both $\Var[X]$ and $X$ on the right (so if $X$ on the left represents the initial state and $X$ on the right represents the final state, then we have returned the value of the state and left the state unchanged).  If player $O$ starts with the move $w_y$, return $\checkmark$ and then copy the value $y$ over to $X$ on the right, ignoring the value of $X$ on the left.

Note that we may model multiple iterations of the state transformer as coalgebras:
\[
...
Now we come to the key point of the construction: if $A$ is a game, then the Hyland exponential $\oc A$ \cite{hyland1997games} has the structure of the \emph{final coalgebra} for the functor $A \sequoid\blank$.  Therefore, following \cite{jacobs}, any coalgebra $S\to A\sequoid S$ (i.e., any state transformer) gives rise to a morphism $S\to\oc A$ that encapsulates the explicit state of the state transformer into an implicitly stateful history-sensitive strategy $S\to\oc A$ that corresponds to repeatedly invoking the state transformer, passing the state from one invocation to the next.  

Indeed, the state transformer we defined on $X\implies\Var[X]\sequoid X$ gives rise to a morphism $X\to\oc\Var[X]$ that gives us the behaviour of a storage cell with initial value given by the copy of $X$ on the left; composing this with our desired initial value on $X$ gives us the strategy $\cell$.  The difference now is that, rather than reason directly about the implicit strategy $\cell$, we may reason about the explicit state in our state transformer and then use coalgebraic methods to talk about $\cell$.  

It is clear that the key fact in the construction is that the final coalgebra for $A\sequoid\blank$ coincides with the exponential $\oc A$, which has the structure of the \emph{cofree commutative comonoid} on $A$.  We will begin by introducing the general idea of using coalgebra to define higher-order stateful objects, using a construction on the category $\Rel$ of sets and relations.  This construction is simple to define, but the final coalgebra in that context does not give us a good model of the exponential: it gives us a cofree comonoid, but not a commutative one.  This motivates us to study state transformers using game semantics and the sequoid operator.  

We review the definition of a \emph{sequoidal category} from \cite{laird02}: a category-theoretic formalization of the sequoid operator akin to the formalization of the tensor product $\tensor$ by the definition of a \emph{monoidal category}.  We illustrate the definition by showing how the sequoidal structure allows us to model stateful programs as morphisms into the final coalgebra for the functor $A\sequoid\blank$, using the example of the $\cell$ strategy in the category of games.  

We spend the rest of the paper discussing the question of what assumptions we need in order to prove that the final coalgebra for $A\sequoid\blank$ coincides with the cofree commutative comonoid on $A$.  The explicit formula for the cofree commutative comonoid given by Melli\'es, Tabareau and Tasson \cite{MelliesCofCommCom} provides a way to link the two constructions that works for the basic category of games; however, it is not powerful enough to prove that the final coalgebra and the cofree commutative comonoid are the same in the category of games with infinitary winning conditions.  

We turn instead to a purely sequoidal/coalgebraic proof that the final coalgebra $\oc A$ for $A\sequoid\blank$ is the carrier of the cofree commutative comonoid on $A$.  In this case, we will need to assume that our category has a Cartesian structure and also assume an additional hypothesis: that if $A$ and $B$ are objects, then a certain natural morphism $\oc A \tensor \oc B \to \oc (A\times B)$ is an isomorphism.  We will refer to this as the \emph{strong monoidal hypothesis}, since it can be used to show that $\oc\blank$ gives rise to a strong monoidal functor from the category with its Cartesian structure to the category with its monoidal functor.

We show that the strong monoidal hypothesis holds whenever $\oc A$ is a \emph{bifree algebra} for $A\sequoid\blank$: that is, it is simultaneously an initial algebra and a final coalgebra.  

We finish with a non-example: the sequoidal category of games with transfinite plays.  In this setting the final coalgebra for $A\sequoid\blank$ does not carry the structure of the cofree commutative comonoid over $A$.  By our discussion above, note that the final coalgebra for $A\sequoid\blank$ has the structure of $\omega$-many copies of the game $A$, where we pass the state from $A_i$ to $A_{i+1}$:
\tikzset{dotsstyle/.style={draw=none}}
\[
  \begin{tikzpicture}
    \matrix[matrix of nodes, column sep=36pt, inner xsep=1ex, name=m, nodes={draw=black}] {
      $A_0$ & $A_1$ & $A_2$ & |[dotsstyle]| $\cdots$ \\ };
    \path[commutative diagrams/.cd, every arrow, every label, labels={font size=tiny}]
      (m-1-1) edge node {state} (m-1-2)
      (m-1-2) edge node {state} (m-1-3)
      (m-1-3) edge node {state} (m-1-4);
  \end{tikzpicture}
\]

For finite interactions, we can use this to model the exponential, in which repeated access to the resource $A$ is allowed, by passing an empty state in between successive invocations of $A$.  In the transfinite case, however, there might not be enough copies of $A$ in the final coalgebra to satisfy our needs: consider a program that, over time $\omega$, uses the resource $A$ infinitely many times.  If we carry on into transfinite time, the program may continue to request access to the resource $A$.  But this is no longer possible if we are using the final coalgebra for our `exponential', since we are only able to construct a sequence of invocations of $A$ of length $\omega$: there is no predecessor copy of $A$ to receive the (empty) state from.

We shall demonstrate why our earlier approaches fail in this context: the Melli\'es-Tabareau-Tasson construction fails badly in this case, just as it fails for win games and winning strategies (which may be viewed as games of length $\omega+1$).  Similarly, the strong monoidal hypothesis does not hold, since a transfinite interleaving of two sequences of length $\omega$ may have length greater than $\omega$.  
...
The games model satisfies an analog of monoidal closedness, but we will not give that here (see \cite{laird02}).  We see that $\G$ is a prototype example of a distributive, decomposable sequoidal category.  
...
\begin{proposition}Any strong distributive decomposabe sequoidal category has all symmetric tensor powers, and these are preserved by the tensor.  
\end{proposition} 
\begin{proof}
      ...
      & \oc A \tensor \oc A \arrow[r, "\der\tensor\id"]
        & A \tensor \oc A \arrow[r, "\sigma\tensor\id"]
          & B \tensor \oc A \arrow[r, "\dec"] \arrow[d, dotted, "\id\tensor\oc\sigma"]
            & B \sequoid \oc A \;\times\; \oc A \sequoid B \arrow[d, "\id\sequoid\oc\sigma\;\times\;\oc\sigma\sequoid\id"] \\
    \oc B \arrow[r, "\mu"] \arrow[dd, "\oc \tau"']
      & \oc B \tensor \oc B \arrow[rr, "\der\tensor\id"]
        &
          & B \tensor \oc B \arrow[r, "\dec"] \arrow[d, "\tau\tensor\id"]
            & B \sequoid \oc B \;\times\; \oc B \sequoid B \\
    %
      &
        &
          & C \tensor \oc B \arrow[r, "\dec"] \arrow[d, dotted, "\id\tensor\oc\tau"']
            & C \sequoid \oc B \;\times\; \oc B \sequoid C \arrow[d, "\id\sequoid\oc\tau\;\times\oc\tau\sequoid\id"] \\
    \oc C \arrow[r, "\mu"]
