...
\keywords{Game semantics, Linear logic, Transfinite games, Sequoid operator}% mandatory: Please provide 1-5 keywords
% Author macros::end %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
For example, consider the category $\Rel$ of sets and relations, with symmetric monoidal structure given by the Cartesian product (with unit $I$, the singleton set $\{*\}$). This has finite (bi)products (disjoint unions), so we may define the functor $F(A,S) = (A \otimes S) \oplus I$. For any object (set) $A$, let $A^*$ be the set of finite sequences of elements of $A$, and let $\alpha:A^* \rightarrow F(A,A^*)$ be the morphism $\{(\varepsilon,\inr(*))\}\cup  \{(aw,\inl(a,w))\suchthat a \in A,w \in A^*\}$. It is straightforward to show that: 
\begin{lemma}[\cite{Hasuo07generictrace}] $(A^*,\alpha_A)$ is the final coalgebra  for $F(A,\_)$.
\end{lemma}
Since we have a natural transformation $J\from F(A,S) \rightarrow F(A,S\oplus 1)$, we may encapsulate the state in the  state transformer $f:S \rightarrow A \otimes S$ by taking the \emph{anamorphism} of $\tilde{f}=[f;\inl_{A,S};J, \inr]:S\oplus 1 \rightarrow F(A,S\oplus 1)$, --- i.e. the unique $F(A,\_)$-coalgebra morphism from $(S,\tilde{f})$ into $(A^*,\alpha_{A})$. This  is  a morphism from $S$ into $A^*$ corresponding to the initial value of the state: by definition, composing it with $\alpha:A^* \rightarrow F(A,A^*)$ (which we can think of as  \emph{invoking} our stateful object) returns a copy of $f$ together with the encapsulated morphism with updated internal state.   

 Distributivity of $\oplus$ over $\otimes$ gives us a morphism $F(A,S) \oplus F(A',S)\to F(A \oplus A',S)$.  This allows us to construct stateful objects compounded of a series of methods sharing access to a common state. 
