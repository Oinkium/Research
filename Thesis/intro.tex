...
However, there is no guarantee that we are able to prove observational equivalence this way: the terms $M$ and $N$ might be observationally equivalent despite having distinct denotations.  
...
This means that if two terms are observationally equivalent, then we can always prove that they are by showing that they have the same denotation.
...
Unfortunately, this stops working for PCF itself.  
...
Continuity of composition says that least upper bounds with respect to this order are preserved by function application.  
...
Having proved computational adequacy for this new language, we then use operational methods to find a way to identify terms that should be observationally equivalent in the original sense.  
...
In order to introduce the last problem that we face with countable nondeterminism, we note that there are two different reasons why an element of a denotational model (e.g., a strategy) might not be definable in a particular language.  
...
Historically, the study of Full Abstraction has not been too concerned with non-definable elements of the second kind: the reason is that they do not usually play a role in determining the intrinsic equivalence relation, since such a strategy is necessarily not compact.  
