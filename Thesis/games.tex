...
\begin{figure}
  \begin{mathpar}
    ...
    \and
    \Gamma,s\ts x \gets n \oopto \Gamma,(s\vert x\mapsto n)\ts \skipp
    \and
    \Gamma,s\ts \oc x \oopto \Gamma,s\ts s(x)
    \\
...
\begin{lemma}
  Suppose that $\Gamma,s\ts M \oopto \Gamma,\Delta,s'\ts N$, where $M,N$ have type $T$.
  Then the following diagram commutes.
  \[
    \begin{tikzcd}[column sep=28pt]
      ...
      {\oc S_{\Gamma,\Delta}} \arrow[d, "{\cell^{\Gamma,\Delta}}"']
        &
          &
            & \deno T \tensor \oc V_\Gamma \arrow[d, "{\wk_{\deno T,\oc \V_{\Gamma,\Delta}}}"] \\
