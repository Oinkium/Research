...
\subsection{Tree Embeddings and Zigzag Strategies}
...
\end{proposition}
\begin{proof}
  By Proposition \ref{PropProductOfTreeEmbeddings}, the morphism $\langle (A_1\sequoid(\cdots\sequoid(A_n \sequoid\pr_i)\cdots))\rangle$ is given by the tree embedding $\phi\from P_{\prod_i (A_1 \sequoid(\cdots\sequoid(A_n\sequoid B_i)\cdots))} \to P_{A_1\sequoid(\cdots\sequoid(A_n\sequoid \prod_iB_i))}$ defined as follows.
  \[
    \phi(s) = \begin{cases}
      \epsilon & \text{if $s=\epsilon$} \\
      [\inj_{A_1,\cdots,A_n},\inj_{B_j}]_*(s) & \text{if $s$ begins with a move in the $j$-th component}
    \end{cases}
    \]
...
\subsection{Denotational Semantics of Idealized Algol}
...
\subsection{Order-Enrichment of $\G$}
...
\begin{proposition}
  The partial order of strategies for $A$ is directed-complete.  
...
\subsection{Big-Step Operational Semantics of Idealized Algol}
...
\subsection{Small-Step Operational Semantics of Idealized Algol}

We also give a small-step operational semantics for Idealized Algol, which will sometimes be easier to work with.  

This time, instead of defining a relation $\Gamma,s\ts M\converges c,s'$, we define a relation $\Gamma,s\ts M\opto \Gamma',s'\ts M'$, where
\begin{itemize}
  \item $\Gamma,\Gamma'$ are $\Var$-contexts; 
  \item $s$ is a $\Gamma$-store and $s'$ a $\Gamma'$-store;  and
  \item $\Gamma\ts M$, $\Gamma'\ts M'$ are Idealized Algol terms-in-context.
\end{itemize}

The definition of the relation is in Figure \ref{FigIaSsOpSem}.
As an auxiliary definition, we need the notion of an \emph{evaluation context} (see \cite{Felleisen}).
This is a single-holed context defined inductively by the following BNF formula, where $M$ ranges over IA terms.
\begin{center}
\parbox{0.7\textwidth}{
\begin{mathpar}
  E \Coloneqq - \mid \suc E \mid \pred E \mid E;M \mid \If E\Then M \Else M \mid \IfO E \Then M \Else M \mid !E \mid M \gets E \mid E \gets n
\end{mathpar}}
\end{center}

\begin{figure}
  \begin{mathpar}
    \inferrule*{\Gamma,s\ts M \opto \Gamma',s',M'}{\Gamma,s\ts E[M] \opto \Gamma',s'\ts E[M']}
    \\
    \inferrule*{ }{\Gamma,s\ts (\lambda x.M)N \opto \Gamma,s\ts M[N/x]}
    \and
    \inferrule*{ }{\Gamma,s\ts \Y M \opto \Gamma,s\ts M(\Y M)}
    \and
    \inferrule*{ }{\Gamma,s\ts \succ n \opto \Gamma,s\ts n+1}
    \and
    \inferrule*{ }{\Gamma,s \ts \pred n+1 \opto \Gamma,s\ts n}
    \and
    \inferrule*{ }{\Gamma,s\ts \pred 0 \opto \Gamma,s\ts 0}
    \and
    \inferrule*{ }{\Gamma,s\ts \skipp;M \opto \Gamma,s\ts M}
    \and
    \inferrule*{ }{\Gamma,s\ts \If \true \Then N \Else P \opto \Gamma,s\ts N}
    \and
    \inferrule*{ }{\Gamma,s\ts \If \false \Then N \Else P \opto \Gamma,s\ts P}
    \and
    \inferrule*{ }{\Gamma,s\ts \IfO 0 \Then N \Else P \opto \Gamma,s\ts N}
    \and
    \inferrule*{ }{\Gamma,s\ts \IfO n+1 \Then N \Else P \opto \Gamma,s\ts P}
    \and
    \inferrule*{ }{\Gamma,s\ts x \gets n \opto \Gamma,(s\vert x\mapsto n)\ts \skipp}
    \and
    \inferrule*[right={$s(x)=n$}]{ }{\Gamma,s\ts \oc x \opto \Gamma,s\ts n}
    \\
    \inferrule*{ }{\Gamma,s\ts \neww \lambda x.M \opto \Gamma,x,(s\vert x\mapsto 0)\ts M}
    \\
    \inferrule*{ }{\Gamma,s \ts \mkvar W\,R \gets n \opto \Gamma,s\ts Wn}
    \and
    \inferrule*{ }{\Gamma,s \ts \oc\mkvar W\,R \opto \Gamma,s\ts R}
  \end{mathpar}
...
\begin{proposition}
  Let $\Gamma$ be a $\Var$-context, let $s$ be a $\Gamma$-store, and let $\Gamma\ts M$ be an Idealized Algol term-in-context.  
  If $s'$ is another $\Gamma$-store and $\Gamma\ts c$ is a canonical form, then $\Gamma,s\ts M\converges c,s'$ if and only if there is some sequence
  \[
    \Gamma,s\ts M \opto \Gamma_1,s^{(1)},M_1 \opto \cdots \opto \Gamma_n,s^{(n)},M_n \opto \Gamma'',s'',c\,,
    \]
  where $\Gamma_1,\cdots,\Gamma_n,\Gamma''$ are $\Var$-contexts, $s^{(i)}$ is a $\Gamma_i$-store, $s''$ is a $\Gamma''$-store, $\Gamma_i\ts M_i$ are Idealized Algol terms in context, and $s''\vert_\Gamma=s'$.
\end{proposition}
\begin{proof}
  First, suppose that we have some sequence $\Gamma,s\ts M\opto\cdots\opto \Gamma'',s'',c$, where $s''\vert_\Gamma=\s'$.  
  We construct a derivation of $\Gamma,s\ts M\converges c,s'$ by induction on the length of this sequence.

  If the sequence has length $0$, then we can use the rule for the canonical form; i.e.
  \[
    \inferrule*{ }{\Gamma,s\ts c\converges c,\skipp}\,.
    \]
  Otherwise, suppose that the sequence starts $\Gamma,s\ts M\converges \Gamma_1,s^{(1)}\ts M_1$, and that we already have a derivation of
  \[
    \Gamma_1,s^{(1)} \ts M_1 \opto c,s''\vert_{\Gamma_1}\,.
    \]
  We split into cases based on the rule used for $\Gamma,s\ts M\converges \Gamma_1,s^{(1)}$.
