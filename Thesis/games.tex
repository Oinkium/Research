...
\begin{proposition}
  ...
  Then there are sequences $\Gamma=\Gamma_1,\cdots,\Gamma_n$, $s=s^{(1)},\cdots,s^{(n)}$, $M=M_1,\cdots,M_n=c$ such that
  \[
...
\begin{lemma}
  The relation $\oopto$ betwen triples $\Gamma,s\ts M$ preserves the composite
  \[
  ...
  \[
    \begin{tikzcd}[column sep=28pt]
      ...
      {\oc \V_{\Gamma,\Delta}} \arrow[d, "{\mu_{\V_{\Gamma,\Delta}}}"']
        &
          &
            &  \deno{T}\sequoid \oc\V_\Gamma \arrow[d, "{\deno{T} \sequoid (\Gamma+0_\Delta)}"] \\
      {\oc \V_{\Gamma,\Delta} \tensor \oc \V_{\Gamma,\Delta}} \arrow[rr, "{\deno{\Gamma,\Delta\ts N} \tensor \oc \V_{\Gamma,\Delta}}"]
        &
          & {\deno T \tensor \oc \V_{\Gamma,\Delta}} \arrow[r, "{\wk_{\deno T,\oc \V_{\Gamma,\Delta}}}"]
            & {{\deno T \sequoid \oc \V_{\Gamma,\Delta}}\,,}
    \end{tikzcd}
    \]
  where $\Gamma+0_\Delta\from \oc \V_\Gamma \to \oc \V_{\Gamma,\Delta}$ is the composite
  \[
    \oc \V_\Gamma \xrightarrow{\runit_{\oc\V_\Gamma}} \oc \V_\Gamma \tensor I \xrightarrow{\oc\V_\Gamma\tensor \oc0} \oc\V_\Gamma \tensor \oc S_\Delta \xrightarrow{\oc\V_\Gamma\tensor \cell^\Delta} \oc \V_\Gamma \tensor \oc \V_\Delta \to \oc \V_{\Gamma,\Delta}\,;
    \]
  i.e., the morphism given by initializing new storage cells to $0$ for all the new variables in $\Delta$.
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \item Now consider the rule
      ...
      \[
        \oc\V_\Gamma \xrightarrow{\Gamma + 0_x} \oc\V_{\Gamma,x} \xrightarrow{\Lambda\inv(\deno{\Gamma \ts \lambda x.M})} \deno{T}\,,
        \]
      where $\Gamma+0_x$ is as defined in the statement of this Lemma.
      Of course, $\Lambda\inv(\deno{\Gamma\ts \lambda x.M})=\deno{\Gamma,x\ts M}$.

      Next, we claim that we have a commutative diagram
      \[
        \begin{tikzcd}
          \oc S_\Gamma \arrow[r, "\cell^\Gamma"] \arrow[d, "{\oc\langle S_\Gamma,0\rangle}"']
            & \oc \V_\Gamma \arrow[d, "\Gamma+0_x"] \\
          {\oc S_{\Gamma,x}} \arrow[r, "{\cell^{\Gamma,x}}"]
            & {\oc \V_{\Gamma,x}\,.}
        \end{tikzcd}
        \]
      Indeed, we have the diagram
      \[
        \begin{tikzcd}[column sep=2pt, row sep=20pt]
          \oc S_\Gamma \arrow[r] \arrow[dddd, "{\oc\langle S_\Gamma,0\rangle}"' description]
            &[5pt] \oc \bN \tensor \cdots \tensor \oc \bN \arrow[r, "\cell\tensor \cdots \tensor \cell"] \arrow[d, "\runit" description]
              &[10pt] \oc\Varr \tensor \cdots \tensor \oc\Varr \arrow[r] \arrow[d, "\runit" description]
                & \oc\V_\Gamma \arrow[d, "\runit" description] \\
          %
            & \oc \bN \tensor \cdots \tensor \oc\bN \tensor I \arrow[r, "\cell \tensor \cdots \tensor \cell \tensor I" yshift=6pt] \arrow[ddd, "\oc\bN \tensor \cdots \tensor \oc\bN \tensor \oc0" description]
              & \oc \Varr \tensor \cdots \tensor \oc\Varr\tensor I \arrow[d, "\oc\Varr \tensor \cdots \tensor \oc\Varr \tensor I" description] \arrow[r]
                & \oc\V_\Gamma \tensor I \arrow[d, "\oc\V_\Gamma\tensor\oc0" description] \\
          %
            &
              & \oc\Varr \tensor \cdots \tensor \oc\Varr\tensor \oc\bN \arrow[r] \arrow[dd, "\oc\Varr\tensor\cdots\tensor\oc\Varr\tensor\cell" description]
                & \oc\V_\Gamma \tensor \oc\bN \arrow[d, "\oc\V_\Gamma\tensor\cell" description] \\
          %
            &
              &
                & \oc\V_\Gamma\tensor\oc\Varr \arrow[d] \\
          {\oc S_{\Gamma,x}} \arrow[r]
            & \oc\bN \tensor\cdots \tensor\oc\bN \tensor\oc\bN \arrow[uur, "\cell\tensor\cdots\tensor\cell\tensor\oc\bN" {description, near start}] \arrow[r, "\cell\tensor\cdots\tensor\cell\tensor\cell"' yshift=-3pt]
              & \oc\Varr \tensor \cdots\tensor\oc\Varr \tensor \oc\Varr \arrow[ur] \arrow[r]
                & {\oc\V_{\Gamma,x}\,.}
        \end{tikzcd}
        \]
      Now the diagram in Figure \ref{FigSoundnessOoptoNew} shows us that the conclusion of the lemma is satisfied for this rule.
      \begin{SidewaysFigure}
        \[
          \begin{tikzcd}[ampersand replacement=\&, column sep=50pt, row sep=40pt]
            I \arrow[r, "\oc\deno s"] \arrow[d, "\oc(\deno s\times 0)"']
              \& \oc S_\Gamma \arrow[dl, "{\oc\langle S_\Gamma,0\rangle}" description] \arrow[dr, "\mu_{S_\Gamma}" description] \arrow[r, "\cell^\Gamma" description]
                \& \oc \V_\Gamma \arrow[rr, "\mu_{\V_\Gamma}"]
                  \&
                    \& \oc \V_\Gamma \tensor \oc\V_\Gamma \arrow[d, "(\Gamma + 0_x)\tensor\oc\V_\Gamma"] \arrow[ddddllll, "(\Gamma+0_x)\tensor(\Gamma+0_x)" description, bend left=12] \\
            {\oc S_{\Gamma,x}} \arrow[dr, "\mu_{S_\Gamma}" description] \arrow[d, "{\cell^{\Gamma,x}}"']
              \&
                \& \oc S_\Gamma \tensor \oc S_\Gamma \arrow[urr, "\cell^{\Gamma} \tensor \cell^{\Gamma}" {description, near start}, bend left=10] \arrow[dl, "{\oc\langle S_\Gamma,0\rangle\tensor\oc\langle S_\Gamma,0\rangle}" description]
                  \& 
                    \& {\oc\V_{\Gamma,x}\tensor\oc\V_\Gamma} \arrow[d, "{\deno{\Gamma,x\ts M}\tensor\oc\V_\Gamma}"] \\
            {\oc\V_{\Gamma,x}} \arrow[dd, "{\mu_{\V_{\Gamma,x}}}"']
              \& {\oc S_{\Gamma,x}\tensor\oc S_{\Gamma,x}} \arrow[ddl, "{\cell^{\Gamma,x}\tensor\cell^{\Gamma,x}}"' {description, near start}, bend right=10]
                \&
                  \&
                    \& \deno T \tensor \oc\V_{\Gamma} \arrow[d, "{\wk_{\deno T,\oc\V_\Gamma}}"] \arrow[ddll, "\deno T \tensor (\Gamma + 0_x)" description, bend left=8] \\
            %
              \&
                \&
                  \&
                    \& \deno T \sequoid \oc\V_\Gamma\arrow[d, "{\deno T \sequoid (\Gamma + 0)}"] \\
            {\oc \V_{\Gamma,x} \tensor \oc \V_{\Gamma,x}} \arrow[rr, "{\deno{\Gamma,x\ts M}\tensor \oc\V_{\Gamma,x}}"]
              \&
                \& {\deno T \tensor \oc\V_{\Gamma,x}} \arrow[rr, "{\wk_{\deno T,\oc\V_{\Gamma,x}}}"]
                  \&
                    \& {\deno T\sequoid\oc\V_{\Gamma,x}}
          \end{tikzcd}
          \]
        \caption{The reduction $\Gamma,s\ts\neww\lambda x.M\oopto\Gamma,x,(s\vert x\mapsto 0)\ts M$ satisfies the conclusion of Lemma \ref{LemSoundnessOopto}.}
    ...
    \item Now consider the rules
      ...
      \[
        ...
        \]
      Now the diagram in Figure \ref{FigSoundnessOoptoContext} tells us that each
      \[
        \oc\deno{s};\cell^{x,\Gamma};\mu_{\V_{x,\Gamma}};(\deno{x,\Gamma\ts M}\tensor\oc\V_{x,\Gamma});\wk_{\deno T,\oc\V_{x,\Gamma}}\,\text{--}
        \]
      i.e., the composites we are trying to show are preserved by the $\oopto$ relation -- may be written as
      \begin{mathpar}
      ...
      and so is determined by the composite
      \[
        \oc\deno{s\vert_x};\cell;\mu_{\Varr};(\deno{x\ts M}\tensor\oc\Varr);\wk_{\deno T,\oc\Varr}\,;
        \]
      i.e., the composite that we would get if $\Gamma$ were empty.
      ...
      Now the commutative diagrams in Figure \ref{FigSoundnessOoptoStorage} deal with these last two cases.\qedhere
      \begin{figure}[htbp]
        \begin{mathpar}
...
\begin{proof}
  ...
  \[
    \begin{tikzcd}
      I \arrow[r, "\deno s"] \arrow[d, "\deno s"']
        & \oc S_\Gamma \arrow[r, "\cell^\Gamma"] \arrow[d, "\lunit_{S_\Gamma}" description]
          & \oc \V_\Gamma \arrow[d, "\lunit_{\oc\V_\Gamma}"'] \\
      \oc S_\Gamma \arrow[d, "\cell"'] \arrow[dr, "\mu_{S_\Gamma}" description]
        & I \tensor \oc S_\Gamma \arrow[r, "I\tensor \cell" description] \arrow[d, "\deno s \tensor \oc S_\Gamma" description]
          & I \tensor \oc\V_\Gamma \arrow[d, "\deno s \tensor \oc\V_\Gamma"] \\
      \oc\V_\Gamma \arrow[d, "\mu_{\V_\Gamma}"'] \arrow[dr, "\mu_{\V_\Gamma}" description]
        & \oc S_\Gamma \tensor \oc S_\Gamma \arrow[d, "\cell\tensor\cell" description] \arrow[r, "\oc S_\Gamma\tensor \cell" description]
          & \oc S_\Gamma \tensor \oc\V_\Gamma \arrow[dl, "\cell\tensor\oc \V_\Gamma"] \\
      \oc\V_\Gamma \tensor \oc\V_\Gamma \arrow[d, "\mu_{\V_\Gamma}\tensor\oc\V_\Gamma"']
        & \oc\V_\Gamma \tensor \oc\V_\Gamma \arrow[d, "\mu_{\V_\Gamma}\tensor\oc\V_\Gamma"]
          & \\
      (\oc\V_\Gamma\tensor\oc\V_\Gamma) \tensor\oc\V_\Gamma \arrow[r, "{\sym^{(23)}_{\oc\V_\Gamma,\oc\V_\Gamma,\oc\V_\Gamma}}" description] \arrow[d, "\deno{\Gamma\ts M}\tensor\oc\V_\Gamma"']
        & (\oc\V_\Gamma\tensor\oc\V_\Gamma) \tensor\oc\V_\Gamma \arrow[r, "(\deno{\Gamma\ts M}\tensor\sigma)\tensor\oc\V_\Gamma"] \arrow[d, "\deno{\Gamma\ts M}\tensor\oc\V_\Gamma" description]
          & (\deno T\tensor A)\tensor\oc\V_\Gamma \arrow[d, "{\wk_{\deno T,A}\tensor\oc\V_\Gamma}"] \\
      (\deno T \tensor \oc\V_\Gamma)\tensor\oc\V_\Gamma \arrow[r, "{\sym^{(23)}_{\deno T,\oc\V_\Gamma,\oc\V_\Gamma}}" description] \arrow[d, "{
\end{proof}
