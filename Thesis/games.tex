...
Next, we define a relation $\Gamma,s,M\oopto\Gamma',s',M'$ as in Figure \ref{FigIaSsOpSem}.

We then define the relation $\opto$ as
\[
  \inferrule*{ \Gamma,s\ts M\oopto \Gamma',s'\ts M'}{\Gamma,s\ts \EE[M] \opto \EE[M']}
  \]
...
Given a $\Gamma'$-store $s$, and a sub-context $\Gamma$ of $\Gamma'$, write $s\vert_{\Gamma}$ for the restriction of $s$ to $\Gamma$.

\begin{lemma}
  Suppose that $\Gamma,s\ts M \oopto \Gamma',s'\ts M'$ and that $\Gamma',s'\ts \EE[M'] \converges c,s''$.  
...
\begin{proof}
  ...
  If $\EE=\EE'\,N$, for some term $N$, then we have $\Gamma',s'\ts \EE'[M']\,N\converges c,s''$.  
  By inspection of the rules in Figure \ref{FigIaOpSem}, the derivation of this must end with a rule of the form
  \[
    \inferrule*{\Gamma',s'\ts \EE'[M'] \converges \lambda x.M'',t \\ \Gamma',t \ts M''[N/x] \converges c,s''}
    {\Gamma',s'\ts \EE'[M']\,N \converges c,s''}\,.
    \]
  Thus, $\Gamma',s'\ts \EE'[M']\converges \lambda x.M'',t$ must be provable for some $M'',t$.
  ...
  \[
    ...
    \]
  Then, because any extra variables in $\Gamma'$ are not mentioned in $\EE'[M]\,N$ or in $c$, we have
  \[
    \Gamma,s\ts \EE'[M]\,N\converges c,s''\vert_\Gamma\,.
    \]
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \item Suppose that the last step in the derivation is
      ...
      \begin{mathpar}
        \Gamma,s \ts M \opto \cdots \opto \Gamma',t',\lambda x.M'
        \and
        \Gamma,s' \ts M'[N/x]\opto \cdots \opto \Gamma'',t'',c\,,
      \end{mathpar}
      ...
      \begin{center}
        \parbox{0.8\textwidth}{
        \begin{mathpar}
          \Gamma,s \ts MN \opto \cdots \opto \Gamma',t',(\lambda x.M')N \opto \Gamma',t',M'[N/x] \opto \cdots \opto \Gamma'\cup\Gamma'',t''\setminus t',c\,,
        \end{mathpar}}
      \end{center}
      where $t''\setminus t'$ is the $\Gamma'\cup\Gamma''$-store that agrees with $t''$ on $\Gamma''$ and with $t'$ on $\Gamma'\setminus\Gamma''$.
    ...
    \item Suppose that the last step in the derivation is
      ...
      \begin{mathpar}
        \Gamma,s\ts E \opto \cdots \opto \Gamma',t',n
        \and
        \Gamma,s'\ts V \opto \cdots \opto \Gamma'',t'',x\,,
      \end{mathpar}
      where $t'\vert_{\Gamma}=s'$ and $t''\vert_{\Gamma}=s''$.
      ...
      \begin{center}
        \parbox{0.8\textwidth}{
        \begin{mathpar}
          \Gamma,s\ts V\gets E \opto \cdots \opto \Gamma',t',V\gets n \opto \cdots \opto \Gamma'\cup\Gamma'',t''\setminus t',x\gets n \opto \Gamma'\cup\Gamma'',(t''\setminus t'\vert x\mapsto n)\,,
        \end{mathpar}}
      \end{center}
    ...
    \item Suppose that the last step in the derivation is
      ...
      \[
        \Gamma,s\ts V \opto \cdots \opto \Gamma',t'\ts x\,,
        \]
      ...
      \[
        \Gamma,s\ts \oc V \opto \cdots \opto \Gamma',t'\ts \oc x \opto \Gamma',t'\ts t'(x)\,,
        \]
      where $t'(x)=s'(x)$.
    \item Lastly, suppose that the last step in the derivation is
      ...
      \[
        \Gamma,x,(s\vert x\mapsto 0) \ts M \opto \cdots \opto \Gamma',x,(t'\vert x\mapsto n),c\,,
        \]
      where $t'\vert_\Gamma=s'$.  
      Then we may add a term at the beginning to give us
      \[
        \Gamma,s \ts \neww \lambda x.M \opto \Gamma,x,(s\vert x\mapsto 0) \ts M \opto \cdots \opto \Gamma',x,(t'\vert x\mapsto n),c\,,
        \]
...
Given $\Gamma$, we have a morphism $\cell^n\from \oc S_\Gamma \implies \oc V_\Gamma$, given by
\[
  ...
  \]
\begin{lemma}
  For $j=1,\cdots,n$, the following diagram commutes.
  \[
    \begin{tikzcd}
      \oc S_\Gamma \arrow[r, "\cell^n"] \arrow[d, "\oc\pr_j"']
...
\begin{lemma}
  ...
  Then there is a game $A$ and strategies $\sigma\from \oc\V_\Gamma \implies A$, $\tau\from\deno{T}\sequoid A \implies \deno{S}$ such that $\tau$ is a strict strategy and the denotation $\deno{\Gamma\ts\EE[M]}$ factors as
  \[
  ...
  \begin{itemize}
    ...
    \item If $\EE = \If \EE' \Then N \Else P$ for a context $\EE'$ of type $\bool$, where $N$ and $P$ are terms of type $X\in\{\bool,\com,\nat\}$, then write $Y=\bool$ and $\eta = \If_X$.  
      ...
      \begin{SidewaysFigure}
        \small
        \[
          \begin{tikzcd}[ampersand replacement=\&, column sep=78pt, row sep=20pt]
          ...
          \]
          \normalsize
        \caption[The property in Lemma \ref{LemEvContexLemma} is preserved by conditionals.]{The property in Lemma \ref{LemEvContexLemma} is preserved by sequencing and variable assignment.
        We use the fact that $\eta\in\{\If_X,\IfO_X\}$ is a strict strategy, and that the $\Lambda_s\inv$ is a function from strict strategies to strict strategies, so that $\Lambda_s\inv(\eta)$ is well-defined.}
