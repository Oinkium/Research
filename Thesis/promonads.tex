...
\begin{definition}
  ...
  \[
    F,G\from \M \to \N\,,
    \]
  a \emph{multinatural transformation} $\phi\from F \Rightarrow G$ is given by morphisms $\phi_a \from F a \to G a$ for each object $a$ of $\M$, such that if $f\from a_1,\cdots a_n\to b$ is any morphism in $\M$, then the following diagram commutes.
  \[
...
\begin{remark}
  ...
  \[
    ...
    \]
  However, this multicategory is not representable in general, particularly in the cases when we are working with enriched multicategories (not defined here), where the enriching multicategory is not cocomplete, or when the category $\M$ is not the enriching category.
\end{remark}
...
But now, by Proposition \ref{PropPromonadsAreCategories}, this promonad is the same thing as an $[\X,\Set]$-enriched category that has the same objects as $\C$ and admits an identity-on-objects $[\X,\Set]$-enriched functor out of ${\omega_X}_*(\C)$.  
