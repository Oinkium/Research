...
\begin{proposition}
  ...
  I.e., let
  \begin{mathpar}
    (V\from\Omega \to X,\sigma\from 1 \to (X \to A))
    \and
    (W\from\Omega \to Y,\sigma'\from 1 \to (Y \to A))
  \end{mathpar}
...
\begin{proof}
  ...
  \begin{mathpar}
    \phi_{\sigma,\tau}\from (X\times Y)^* \to X^* \times Y^*
    \and
    \Phi_{\sigma,\tau}\from (X\times Y)^* \to (X\times Y)^* \times (X \times Y)^*
  \end{mathpar}
