...
{\setlength{\IEEEnormaljot}{20pt}%
  ...
  By the definition of $\mu$, we know that if the $i$-th pair of moves in $\t\vert_{\ul X,\ul Y}$ is $qx$ for $x\in X$, then the $i$-th pair of moves in $t\vert_{\ul{X\times Y}}$ is $q(x,y_0)$ for some $y_0$, and if the $i$-th pair of moves in $\t\vert_{\ul X,\ul Y}$ is $qy$ for $y\in Y$, then the $i$-th pair of mvoes in $t\vert_{\ul{X\times Y}}$ is $q(x_0,y)$ for some $x_0\in X$.
...
This is the approach taken by Danos and Harmer in \cite{DanosHarmer} -- the original game semantics for a probabilistic variant of Algol.  
...
\[
  \sigma(s) \ge \sum_{sab\in\L_A} \sigma(sab)\,.
  \]
These quantites $\sigma(s)$ take the role of our $\bP(s\in\sigma)$, but now they are part of the \emph{definition} of the strategy $\sigma$, rather than being a calculated quantity.
