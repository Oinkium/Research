...
\begin{proposition}[\cite{AllGoodCategories}]
...
\begin{proof}
  ...
  See \ref{AllGoodCategories} for full details.
\end{proof}
...
We then conclude by changing base through this colimit functor.  
...
\end{definition}
\begin{example}
  Given a monoidal category $\X$, write $\BB \X$ for the bicategory having a single object $*$, where $\BB\X(*,*)$ is the category $\X$ and composition of $1$-cells $x,y \from * \to *$ is given by
  \[
    x;y = x \tensor y\,.
    \]
  If $\C = \BB \X$, $\D = \BB \YY$ are the delooping bicategories of monoidal categories $\X, \YY$, then a lax functor $\C \to \D$ is the same thing as a lax monoidal functor $\X \to \YY$.  
\end{example}

\begin{example}
  More generally, a lax functor $F\from \BB \X \to \D$ is the same as a lax monoidal functor from $\X$ to the monoidal category of $1$-cells $F(*) \to F(*)$.
...
\begin{definition}
  ...
  Then an \emph{oplax cocone under $F$} is given by an object $d$ of $\D$ (called the \emph{tip} of the cocone), together with $1$-cells
  \[
    l_c \from F(c) \to d
    \]
...
\begin{corollary}
  ...
  In particular, there is a functor $J\from \C \to \C/\X$ and a natural transformation
  \[
...
\begin{remark}
  Here, $J\from \C \to \C/\X$ is the identity on objects and sends a morphism $f \from a \to b$ in $\C$ to the \Mellies morphism
  \[
    a \xrightarrow{f} b \xrightarrow{e_b} I.b\,,
    \]
  while the component $\phi_{x,a}$ of $\phi$ is the identity
  \[
    x.a \to x.a\,,
    \]
  considered as a \Mellies morphism $x.a \to a$.
\end{remark}
...
\begin{definition}
  ...
  \begin{mathpar}
    m_{x,y,a} \from x.y.a \Rightarrow (x\tensor y).a \from \X \times \X \times \C \to \C
    \and
    e_a \from a \Rightarrow I.a \from \C \to \C
  \end{mathpar}
...
\begin{proposition}
  Let a symmetric monoidal category $\X$ act on a Cartesian category $\C$ via a symmetric monoidal lax action.  
  Suppose that \textbf{either}
  \begin{itemize}
    \item for every object $x$ of $\X$ there are morphisms $f\from x \to x\tensor x$ and $f_0 \from x \to I$ that make the following diagrams commute;
      \begin{mathpar}
        \begin{tikzcd}
      ...
      \end{mathpar}
    \textbf{or}
  \item for every object $x$ of $\X$ there is are morphisms $g \from x \tensor x \to x$ and $g_0 \from I \to x$ that make the following diagrams commute.
    \begin{mathpar}
      \begin{tikzcd}
    ...
    \end{mathpar}
  \end{itemize}
  Then the monoidal structure on $\C/\X$ is Cartesian.
  \label{PropCartesianness}
\end{proposition}
\begin{proof}
  In either case, the morphisms $f,f_0$ or $g,g_0$ mediate between the composites from our earlier discussion.
\end{proof}

Note that these results is not the sharpest possible: there may well be more complicated zigzags mediating between the morphisms.
...
\begin{proof}
  ...
  \begin{SidewaysFigure}
    \begin{mathpar}
      \begin{tikzcd}[ampersand replacement=\&]
        %
        \&[-37pt] \left(\colim_{x\from \X}\limits x.a \tensor \colim_{y\from \X}\limits y.b\right) \tensor \colim_{z \from \X}\limits z.c \arrow[r, bend left=15, yshift=-1em, "\assoc"] \arrow[d]
            \&[-26pt] \colim_{x \from \X}\limits x.a \tensor \left(\colim_{y\from \X}\limits y.b \tensor \colim_{z \from \X}\limits z.c \right) \arrow[d]
              \&[-37pt] \\
        %
  ...
  \begin{figure}
    \begin{mathpar}
      ...
      \begin{tikzcd}
        %
          &[-18pt] |[alias=Z]| \colim_{z\from \X}\limits z.(a \tensor 1)
            &[-20pt] \\
        |[alias=Y]| \colim_{x\from \X}\limits x.a \arrow[r, "\Colim\runit.\runit" yshift=4pt] \arrow[ur, "\Colim \id.\runit", from=Y.north, to=Z.west, bend left=20] \arrow[d, "\Colim\runit" description] \arrow[dd, "\runit"', bend right=60]
        ...
        \colim_{x\from \X}\limits x.a \tensor 1 \arrow[r, "\Colim \id \tensor e" yshift=2pt]
        ...
        \left(\colim_{x \from \X}\limits x.a\right) \tensor 1 \arrow[u, Sim] \arrow[r, "\id \tensor e" yshift=2pt]
...
\begin{proof}
  ...
  \begin{IEEEeqnarray*}{Cl}
    ...
    \xrightarrow{\makebox[141pt]{$\int^{y,z} m_{jy,jz,[A,B],[B,C]}\times \X(y\tensor z,x)$}} &
    \int^{y,z}[jy \times jz,[A,B]\times [B,C]] \times \X(y,\tensor z,x) \\
    \xrightarrow{\makebox[141pt]{$\int^{y,z} [m^j_{y,z}, ;] \times \X(y\tensor z,x)$}} &
