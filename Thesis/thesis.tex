...
\begin{proof}
  ...
  Conversely, if $\deno M \not\smam \deno N$, then without loss of generality there is some $\alpha\from\deno T \to \bC$ such that $\alpha;\deno M \downarrow_{\must}$ and $\alpha;\deno N\not\downarrow_{\must}$.  
  But by universality $\alpha;\deno M = \deno{LM}$ and $\alpha;\deno N=\deno{LN}$ for some term $L\from T \to \com$ of IA.  
  Therefore, the result follows from Corollary \ref{CorAdeqMust}.
\end{proof}

\begin{example}
  It was proven in \cite{hoPcf} that every innocent strategy between the denotations of PCF types is innocently intrinsically equivalent to the denotation of some term of PCF.  
  A sharper result (unpublished, but see \cite[\sec 7.1.5]{LongleyBook}), which allows us to forget the intrinsic equivalence part, is that every innocent strategy between the denotations of PCF terms is the denotation of some term of the language formed by augmenting PCF with a constant called $\lett$ or $\byval$ whose 
