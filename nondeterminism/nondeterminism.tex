...
\begin{example}
  ...
  We have a nondeterministic strategy $?$ for $\bN\cong I\implies\bN$ where the player nondeterministically chooses any number - so $? = P_{\bN}^P$ as sets of positions.  $?$ is compact because all plays in $?$ have length at most $2$.  
...
The numerals $\underline{n}$ will each be interpreted as the strategy for $\bN$ that always returns the value $n$.  The nondeterministic oracle $?$ will be interpreted as the strategy $?$ given at the level of trace sets by $?=P_{\bN}^P$.  None of these strategies have any divergent positions.
...
\begin{lemma}
  \label{jimsLemma}
  Let $M$ be a reducible program of type $T$.  If $M\opto N$ then $\deno{N}\exle\deno{M}$.
  \begin{proof}
    This is easy to show inductively by analysing the reduction rules.
  \end{proof}
\end{lemma}
