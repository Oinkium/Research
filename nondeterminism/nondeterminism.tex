...
\begin{itemize}
  ...
  \item $D_\sigma\subset T_\sigma$ denotes the set of \emph{divergences} in $T_\sigma$; that is, the set of those $O$-positions in $T_\sigma$ for which Player $P$ may elect to diverge rather than respond.  Accordingly, $\zeta_A(s)=O$ for all minimal $s\in D_\sigma$.  If a trace has a divergent prefix, then it is divergent, so we shall follow Roscoe and insist that $D_\sigma$ be postfix-closed in $P_A^O$ (\cite{RoscoeCspInfinite}, cited in \cite{mcCHFiniteND}).
\end{itemize}
...
Let $A$ be a game and let $F$ be a non-empty set of strategies for $A$.  We define a strategy $\exlub F$ for $F$ by setting
\begin{gather*}
...
\begin{proposition}
  If $A$ is a game and $F$ is a set of strategies for $A$ then $\exlub F$ is a strategy for $A$.
  \begin{proof}
    Certainly $T_{\exlub F}$ is non-empty and prefix-closed and $D_{\exlub F}$ is postfix-closed.  Suppose $s\in T_{\exlub F}$ is a $P$-position.  Then $s\in T_\sigma$ for some $\sigma\in F$.  So if $sa\in P_A$ for some move $a$, then $sa\in T_\sigma$ and therefore $sa\in T_{\exlub F}$.  

    Let $s\in D_{\exlub F}$.  Then $s\in D_\sigma$ for all $\sigma\in F$.  It follows that 
  \end{proof}
\end{proposition}
